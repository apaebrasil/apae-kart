...
import { drivers, races, results, calcStandings } from "@/data/championship";
      ...
      <div className="min-h-dvh bg-asphalt flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center container py-20">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-6">Área restrita à direção de prova. Use a senha de demonstração <code className="text-volt font-mono">admin</code>.</p>
            <Label htmlFor="pw" className="font-display uppercase tracking-widest text-xs">Senha</Label>
            <Input id="pw" type="password" value={pw} onChange={(e) => setPw(e.target.value)} className="mt-2 bg-asphalt" />
            <Button type="submit" className="w-full mt-6 font-display uppercase tracking-widest">Acessar Painel</Button>
          </form>
        </main>
        <Footer />
      </div>
    ...
    <div className="min-h-dvh bg-asphalt flex flex-col">
      ...
      <main className="container py-12 flex-1">
        <div className="flex items-end justify-between mb-8">
          ...
          <Button variant="outline" onClick={onLogout} className="font-display uppercase tracking-widest text-xs">Sair</Button>
        </div>
...
const DriverForm = () => {
  ...
  return (
    <form onSubmit={onSubmit} className="bg-tarmac border border-border p-6 space-y-4">
      ...
      <div>
        <Label htmlFor="bio" className="font-display uppercase tracking-widest text-xs">Bio</Label>
        <Textarea id="bio" className="mt-2 bg-asphalt" rows={3} />
      </div>
      <Button type="submit" className="font-display uppercase tracking-widest">Cadastrar Piloto</Button>
    </form>
  );
};
    ...
    <form onSubmit={onSubmit} className="bg-tarmac border border-border p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Select>
            <SelectTrigger className="mt-2 bg-asphalt"><SelectValue placeholder="Selecione a prova" /></SelectTrigger>
            <SelectContent>{upcoming.map((r) => <SelectItem key={r.id} value={r.id}>R{r.round} · {r.name}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div>
          <Label className="font-display uppercase tracking-widest text-xs">Piloto</Label>
          <Select>
            <SelectTrigger className="mt-2 bg-asphalt"><SelectValue placeholder="Selecione o piloto" /></SelectTrigger>
            <SelectContent>{drivers.map((d) => <SelectItem key={d.id} value={d.id}>#{d.number} {d.name}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <Field label="Posição Final" id="pos" type="number" min={1} max={20} />
        <Field label="Melhor Volta (mm:ss.mmm)" id="bl" placeholder="00:42.318" />
      </div>
      <Button type="submit" className="font-display uppercase tracking-widest">Lançar Resultado</Button>
    </form>
  );
};
...
const NewsForm = () => {
  ...
  return (
    <form onSubmit={onSubmit} className="bg-tarmac border border-border p-6 space-y-4">
      ...
      <div>
        <Label htmlFor="content" className="font-display uppercase tracking-widest text-xs">Conteúdo</Label>
        <Textarea id="content" className="mt-2 bg-asphalt" rows={6} />
      </div>
      <Button type="submit" className="font-display uppercase tracking-widest">Publicar Notícia</Button>
    </form>
  );
};
...
const Field = ({ label, id, ...props }: { label: string; id: string } & React.InputHTMLAttributes<HTMLInputElement>) => (
  <div>
    <Label htmlFor={id} className="font-display uppercase tracking-widest text-xs">{label}</Label>
